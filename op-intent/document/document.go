...
func NewGJSONDocument(contents []byte) (Document, error) {
...
func (m MapDocument) Array() Documents {
	return NewMapDocuments(m.e.([]interface{}))
}
...
func NewMapDocuments(is []interface{}) Documents {
	mps := make(MapDocuments, len(is))
	for i := range is {
		mps[i], _ = NewMapDocument(is[i])
	}
	return mps
}
